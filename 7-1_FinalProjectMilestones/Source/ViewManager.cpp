...
void ViewManager::ProcessKeyboardEvents()
{
	...
	// process different camera angles
...
void ViewManager::PrepareSceneView()
{
	...
	m_pShaderManager->setVec3Value("directionalLight.direction", glm::vec3(-0.8f, -1.0f, -0.3f));

	// Brighter ambient and diffuse to improve lighting visibility
	m_pShaderManager->setVec3Value("directionalLight.ambient", glm::vec3(0.2f));
	m_pShaderManager->setVec3Value("directionalLight.diffuse", glm::vec3(0.85f)); 
	...
	{
		// Orthographic projection: zoom/pan effect can be simulated by changing bounds
		float orthoSize = 20.0f;
		float aspect = (float)WINDOW_WIDTH / (float)WINDOW_HEIGHT;
		projection = glm::ortho(-orthoSize * aspect, orthoSize * aspect, -orthoSize, orthoSize, 0.1f, 100.0f);

		// Reset camera position to look straight on (simulate 2D view)
